...
<#
.SYNOPSIS
    Builds the one-click CameraCalibrationStudio-Setup-<version>.exe installer.

.DESCRIPTION
    1. dotnet publish's the app as a self-contained win-x64 build (no .NET install
       needed on the target machine).
    2. Compiles installer\CameraCalibrationStudio.iss with Inno Setup's ISCC.exe into
...
$projectPath = Join-Path $repoRoot 'CameraCalibrationStudio'
$issPath = Join-Path $PSScriptRoot 'CameraCalibrationStudio.iss'
